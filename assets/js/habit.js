(function () {
    ...
    function SetMinMaxDateOnDatePicker() {
        // Get the current date and format it as a string in 'YYYY-MM-DD' format
        currentDate = moment().format('YYYY-MM-DD');

        // Calculate a date that is 90 days before the current date and format it as a string
        nintyDaysBefore = moment(currentDate).subtract(90, 'days').format('YYYY-MM-DD');
        // console.log(nintyDaysBefore);

         // Set the minimum value of the start and end date pickers to 90 days before the current date
        startDate.setAttribute("max", currentDate)
        endDate.setAttribute("max", currentDate);
        startDate.setAttribute("min", nintyDaysBefore)
        endDate.setAttribute("min", nintyDaysBefore);
